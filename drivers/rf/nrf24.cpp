...
#ifndef RF_IRQ_PIN
#    define RF_IRQ_PIN E6
#endif

#ifndef RF_SCK_DIVISOR
#    define RF_SCK_DIVISOR 2 // 4MHz SCK/8MHz CPU, calculated for Feather 32U4 BLE
#endif
...
static bool sdep_send_pkt(const struct sdep_msg *msg, uint16_t timeout) {
    spi_start(RF_CS_PIN, false, 0, RF_SCK_DIVISOR);
...
static bool sdep_recv_pkt(struct sdep_msg *msg, uint16_t timeout) {
    ...
    do {
        ready = gpio_read_pin(RF_IRQ_PIN);
        if (ready) {
            break;
        }
        wait_us(1);
...
static void resp_buf_read_one(bool greedy) {
    ...
    again:
        if (sdep_recv_pkt(&msg, SdepTimeout)) {
            ...
            if (greedy && resp_buf.peek(last_send) && gpio_read_pin(RF_IRQ_PIN)) {
                goto again;
            }
        }
...
void nrf24_init(void) {
    ...
    gpio_set_pin_input(RF_IRQ_PIN);

    spi_init();

    // Perform a hardware reset
    gpio_set_pin_output(RF_RST_PIN);
    gpio_write_pin_high(RF_RST_PIN);
    gpio_write_pin_low(RF_RST_PIN);
    wait_ms(10);
    gpio_write_pin_high(RF_RST_PIN);
...
void nrf24_task(void) {
    ...
    if (!state.configured && !rf_enable_keyboard()) {
        return;
    }
    resp_buf_read_one(true);
    send_buf_send_one(SdepShortTimeout);

    if (resp_buf.empty() && (state.event_flags & UsingEvents) && gpio_read_pin(RF_IRQ_PIN)) {
...
void nrf24_send_keyboard(report_keyboard_t *report) {
    struct queue_item item;
...
void nrf24_send_consumer(uint16_t usage) {
    struct queue_item item;
...
void nrf24_send_mouse(report_mouse_t *report) {
    struct queue_item item;
...
uint32_t nrf24_read_battery_voltage(void) {
    return state.vbat;
}
